<!DOCTYPE html>
<?php $location = '../'?>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta http-equiv="X-UA-Compatible" content="ie=edge">
	<title> BEVC | Jeunes</title>
	<?php include  $location . 'includes/header.php'; ?>
</head>
<body>
	<header>
		<?php include $location . 'includes/menu.php';  ?>
		<div class="pic_presentation img_bg_jeunes">
			<div class="inner_pic_presentation">
				<h3 class="title_header_block">Equipe Jeunes</h3>
			</div>
		</div>
	</header>
	<main>
		<div class="container_personnal_content">
			<div class="intro">
				<p>Nous vous remercions d'avoir choisi le BEVC pour la pratique du volley ball. 
					Vous aurez la possibilité d'accompagner, d'encourager et de voir évoluer votre enfant.</p>
				<p>Vous trouverez toutes les informations concernant 
					l'équipe jeune mais aussi concernant l'académie.
				</p>
			</div>
			<div class="title">
				<h2>Equipe des jeunes</h2>
			</div>
			<div class="young_content">
				<div class="info_text_section">
					<h4>Inscription / ré-inscription</h4>
				<p>Pour une inscription, plusieurs documents à remplir vous seront demandé: 
				<ul>
					<li>Un document d'affiliation</li>
					<li>Une fiche d'inscription </li>
					<li>Un document relatif à l'esprit et au covoiturage</li>
					<li>Une attestation si votre enfant n'est pas de nationalité belge</li>
					<li>le payement de la cotisation*</li>
				</ul>
				Pour une ré-inscription, seul le payement de la cotisation sera à faire.</p>			
				<p>Afin que votre enfant progresse en même temps que le groupe et dispose des mêmes possibilités que les 
				autres la présence aux entrainements est indispensable. </p>
				<p>Les rencontres permettent aux enfants de prendre plaisir à affronter d'autres joueurs et d'appliquer les acquis 
				en situation de match. </p>
				<p>La participation aux tournois 1 contre 1 et 2 contre 2, sont obligatoires car elles font partie de la formation.
				</p>
				<p>En cas d'impossibilité pour les trajets, le coachs bien que n'organisant pas les co-voiturage facilitent la mise
					 en contact entre les parents pour que les transports se fassent d'initiative.
				</p>
				<p>En cas d'accident, que mon enfant se blesse, veuillez télécharger et imprimer la déclaration d'accident,
					 l'envoyer à la fédération après la visite médicale. Vous devez ensuite prévenir Marc Philippart et votre entraineur.<br>
					<a class="btn_link" href="../documents/Declaration Accident ETHIAS.pdf"><button class="btn btn-download">Déclaration d'accident</button></a>
				</p>
				<p>Lors d'un changement de club, c'est au nouveau club de comlêter et de transmettre la demande de transfert à la fédération.</p>

				<p>Le montant de la cotisation est de :
					<ul>
						<li>280 € pour le premier enfant</li>
						<li>260 € pour le second enfant</li>
						<li>240 € à partir du 3ème enfant</li>
					</ul>
				</p>
				<p>
					Le montant est à verser sur le compte <span class="bank_account">BE52 7512 0433 8009</span> avec comme mention "section junior"
				 	ainsi que le nom et prénom de votre enfant.
					Un étalement de la cotisation est possible en consultant notre trésorier, de préférence par mail :
					Marc Vandame: par mail: <a href="mailto:tresorier@bruxellesestvolley.be">tresorier@bruxellesestvolley.be</a>  ou
				 <a href="mailto:volley@skynet.be">volley@skynet.be</a> par téléphone au:  <a href="tel:+32475908594">+32475908594</a>
				</p>
				 <h4>Contacts</h4>
				 <p>Toutes les informations se font essentiellement via: 
					 <ul>
						 <li><a href="https://teamstuff.com/fr/join/bevc-jeunes/83295cfc">Teamstuff</a></li>
						 <li><a href="mailto:jeunes@bruxellesestvolley.be">Mail pour les autres demanedes d'infos</a></li>
						 <li>Facebook: <a href="https://www.facebook.com/groups/436374373204035/">BEVC section jeunes</a></li>
					 </ul>
				 </p>
         <h4>Les entraineurs</h4>
         <p>
           <ul>
             <li>Guy Bruyr : <a href="tel:+32495876830">+32495876830</a> <a href="mailto:eurostart@skynet.be">eurostart@skynet.be</a></li>
             <li>Fred Vansteen : <a href="tel:+3248635440">+3248635440</a> <a href="mailto:fredvs@proximus.be"></a></li>
             <li>Thibaut Cornillot : <a href="tel:+32477053482">+32477053482</a></li>
             <li>Alain Exposito : <a href="tel:+32475971122">+32475971122</a></li>
             <li>Godefroid Jean : <a href="tel:+32472561999">+32472561999</a></li>
             <li>Marc Philippart (secrétaire) : <a href="tel:+32474778726"></a> <a href="mailto:philippartmarc@skynet.be">philippartmarc@skynet.be</a></li>
           </ul>
         </p>
         <p>En cas d'absence, il est impératif de prévenir votre entraineur!</p>
         <div class="timetable">
          <table class="table table-striped">
            <thead>
              <tr>
                <th scope="col"></th>
                <th scope="col">Mercredi au stade Fallon</th>
                <th scope="col">Samedi au stade Fallon</th>
                <th scope="col">Samedi  à Auderghem</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <th scope="row"></th>
                <td>13h30 - 15h00 : moins de 12 ans</td>
                <td>09h00 - 10h45 : moins de 12 ans</td>
                <td>08h30- 10h15 : P4 dames</td>
              </tr>
              <tr>
                <th scope="row"></th>
                <td>15h00 - 16h30 : minimes / cadettes + débutantes dès 12 ans</td>
                <td>11h00 - 12h45 : minimes / cadettes + débutantes dès 12 ans</td>
                <td>10h15 - 12h00 : P3 Dames</td>
              </tr>
              <tr>
                <th scope="row"></th>
                <td>16h30- 18h00 : P4 Dames</td>
                <td></td>
                <td></td>
              </tr>
            </tbody>
          </table>
          <p>Chaque enfant est succectible de changer de groupe au cours de la saison.</p>
         </div>
         <div class="sporthall">
					<p>
						<ul>
							<li>Salle Malou ( stade Fallon ) : Chemin de Struykbeken 2, 1200 Bruxelles</li>
							<li>Salle Omnisport Auderghem : 3ème étage, Entrée Chaussée de Wavre 1690, 1200 Bruxelles</li>
						</ul>
					</p>
         </div>
				</div>
				<div class="info_image_section">
					<picture>
						<img src="../images/content_img/img_jeunes_3.png" alt="">
					</picture>
				</div>
			</div>
		</div>
	</main>
	<footer>
    <?php include $location . 'includes/footer.php'; ?>
  </footer>
  <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.3/umd/popper.min.js" integrity="sha384-vFJXuSJphROIrBnz7yo7oB41mKfc8JzQZiCq4NCceLEaO4IHwicKwpJf9c9IpFgh" crossorigin="anonymous"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/js/bootstrap.min.js" integrity="sha384-alpBpkh1PFOepccYVYDB4do5UnbKysX5WZXm3XxPqe5iKTfUKjNkCk9SaVuEZflJ" crossorigin="anonymous"></script>  
</body>
</html>